...
async function handleSendMarkdownClick(event, button, messageContainer) {
    ...
    try {
        ...
        chrome.runtime.sendMessage({ action: "sendMarkdown", content: markdownContent }, (response) => {
            // --- START OF FIX ---
            // This block is now much smarter about handling the response.

            if (response) {
                if (response.status === "success") {
                    // Case 1: Everything worked perfectly.
                    button.innerHTML = 'Sent!';
                    button.style.backgroundColor = '#34a853'; // Green
                } else if (response.message === 'AI Bridge server is not running.') {
                    // Case 2: The server is off. This is NOT an error.
                    // We log a simple info message, not a red error.
                    console.log('User action feedback: AI Bridge server is off.');
                    button.innerHTML = 'Server Off';
                    button.style.backgroundColor = '#fbbc05'; // A nice warning yellow/orange
                } else {
                    // Case 3: A real, unexpected error occurred (e.g., file permissions).
                    // This is when a console.error is appropriate.
                    console.error('Failed to send due to a server-side error:', response.message);
                    button.innerHTML = 'Error!';
                    button.style.backgroundColor = '#ea4335'; // Red
                }
            } else {
                // Case 4: The background script failed to respond at all.
            ...
            // Reset the button's appearance after a couple of seconds
            setTimeout(() => {
                button.innerHTML = originalButtonHTML;
                button.style.backgroundColor = originalButtonStyle;
                button.disabled = false;
        ...
        // This catch block handles errors in the UI automation part (e.g., can't find menu)
        console.error('Markdown send process failed:', err);
        button.innerHTML = 'Error!';
        button.style.backgroundColor = '#ea4335';
        setTimeout(() => {
