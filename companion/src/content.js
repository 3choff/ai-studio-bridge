const DESIRED_PLACEHOLDER_TEXT = "Message";
...
/**
 * A single observer to handle all UI changes:
 * - Adding buttons to new messages.
 * - Detecting theme changes.
 * - Fixing the input placeholder.
 */
function observeUIChanges() {
  // Set initial state for placeholder and theme
  ensureStaticInputPlaceholder();
  detectAndApplyTheme();

  const mainObserver = new MutationObserver((mutationsList) => {
    // Run placeholder check on any mutation, as the input can be re-rendered.
    ensureStaticInputPlaceholder();
    ...
    for (const mutation of mutationsList) {
      ...
      if (mutation.type === 'attributes' && mutation.attributeName === 'class' && mutation.target === document.body) {
        detectAndApplyTheme();
        continue; // No need to check for added nodes if it was just a class change
      }

      // Logic for adding buttons to new messages
      if (mutation.type === 'childList') {
        for (const node of mutation.addedNodes) {
          if (node.nodeType === Node.ELEMENT_NODE) {
            // Check if the added node itself is or contains a message container
            const containers = node.querySelectorAll ? node.querySelectorAll('.model-prompt-container') : [];
            if (node.matches && node.matches('.model-prompt-container')) {
                 if (node.parentElement) {
                     waitForStableTextAndInsertButtons(node.parentElement);
                 }
            }
  ...
  // Observe the body for both child additions/removals and attribute changes
...
// Start observing all UI changes
observeUIChanges();
